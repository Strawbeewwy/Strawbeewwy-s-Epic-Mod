...
public class ModRingItemTopaz extends ModRingItem{
    public ModRingItemTopaz(Settings settings) {
        super(settings);
    }
    ...
    public void tick(ItemStack stack, SlotReference slot, LivingEntity entity) {
        ...
        if (friend!= null) {
            if (friend.getPos().isInRange(entity.getPos(),effectiveRadius)) {
                entity.addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, 5 * TPS, effectAmplifier));
                entity.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 5 * TPS, effectAmplifier));
                entity.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 5 * TPS, effectAmplifier));
            }
        }
    }
